...
async def test_poro_state_creates_spawn_when_roll_is_due(
    ...
    assert spawn["tier"] == 3
    assert spawn["reward_amount"] == pytest.approx(24.0)
...
async def test_poro_claim_increases_balance_and_records_snapshot(
    ...
    assert claim_body["reward_amount"] == pytest.approx(24.0)
    assert claim_body["balance"] == pytest.approx(starting_balance + 24.0)
